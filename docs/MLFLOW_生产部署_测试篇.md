...
> | 用例 | 内容 | 状态 |
> | --- | --- | --- |
> | **A** | 记录 3 次运行 | ✅ **已跑通** —— 指标入库，**15 个对象**入 S3 |
> | **B1** | 从 PostgreSQL 直接读指标 | ✅ **已跑通** |
> | B2 / B3 / B4 | 删 Pod / 缩容到 0 / 删集群 | ⬜ 未执行 |
> | C | 产出文件持久性 | ⬜ 未执行（判据已按实测修正，见 [C1](#c1-确认产出文件真的在-s3-里)） |
> | D | 模型注册表往返 | ⬜ 未执行 |
>
> 🔧 **实测修正**：用例 C1 的原判据「`artifact_location` 以 `s3://` 开头」**不成立**。
> MLflow 3.16 在代理模式下存的是 `mlflow-artifacts:/1` 这种代理 URI。详见 C1。
...
| # | 测试 | 判定标准 | 实测 |
| --- | --- | --- | --- |
| A | 记录运行 | 界面上 3 次运行，含指标和产出文件 | ✅ 已通过 |
| B1 | 从 PostgreSQL 读取 | `psql` 返回相同的准确率 | ✅ 已通过 |
| B2 | 删除 Pod | 记录存活，新 Pod 继续提供服务 | ⬜ 未测 |
| B3 | 缩容到 0 | 0 个 Pod 时运行记录数不变 ⭐ | ⬜ 未测 |
| B4 | 删除集群 | 无集群时记录依然存在 | ⬜ 未测 |
| C | 产出文件持久性 | **`aws s3 ls` 看到对象**（不是看 `artifact_location`） | ⬜ 未测 |
...
rm -f test_a_train.py test_d_registry.py        # 可选：测试脚本
